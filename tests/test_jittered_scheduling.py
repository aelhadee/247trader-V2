...
- Jitter clamping to safe bounds (0-20%)- Jitter applied within 0-jitter_pct% range

- Jitter applied within specified range- Jitter telemetry tracked in StateStore
...
from unittest.mock import Mock, patchimport time
...
class TestJitterConfiguration:
    ...
    def test_jitter_loads_from_policy(self):    """Test jitter configuration loads from policy.yaml."""

        """Test jitter_pct loads from policy.yaml."""    loop = TradingLoop(config_dir="config")

        loop = TradingLoop(config_dir="config")    # Should load jitter_pct from policy.yaml loop section (default 10.0)

        assert hasattr(loop, 'loop_jitter_pct')    assert hasattr(loop, 'loop_jitter_pct')

        assert isinstance(loop.loop_jitter_pct, float)    assert isinstance(loop.loop_jitter_pct, float)

        # Should be 10.0 from policy.yaml    assert loop.loop_jitter_pct == 10.0  # From policy.yaml

        assert loop.loop_jitter_pct == 10.0

    

    def test_jitter_clamping_to_max(self):def test_jitter_config_clamping_bounds():
        ...
        with patch.object(TradingLoop, '_load_yaml') as mock_load:    config_dir.mkdir()
            ...
            def mock_yaml_loader(filename):    
                ...
                if 'policy.yaml' in filename:    # Configs with 5% jitter
                    ...
                    return {    (config_dir / "app.yaml").write_text(yaml.dump({

                        'loop': {'interval_seconds': 60, 'jitter_pct': 50.0},  # Try 50%        "app": {"mode": "DRY_RUN"},

                        'risk': {'max_total_at_risk_pct': 15.0},        "exchange": {"api_key": "test", "api_secret": "test", "read_only": True},

                        'execution': {'maker_fee_bps': 40, 'taker_fee_bps': 60},        "logging": {"level": "INFO"}

                        'latency': {'total_seconds': 45.0}    }))

                    }    

                return {'triggers': {}} if 'signals' in filename else {'tiers': {}}    (config_dir / "policy.yaml").write_text(yaml.dump({
            ...
            with patch('tools.config_validator.validate_all_configs', return_value=[]):    }))
                ...
                assert loop.loop_jitter_pct == 20.0  # Clamped to max    (config_dir / "signals.yaml").write_text(yaml.dump({"triggers": {}}))

        (config_dir / "universe.yaml").write_text(yaml.dump({"tiers": {}}))

    def test_jitter_clamping_to_min(self):    

        """Test jitter values are clamped to minimum 0%."""    loop = TradingLoop(config_dir=str(config_dir))

        with patch.object(TradingLoop, '_load_yaml') as mock_load:    assert loop.loop_jitter_pct == 5.0

            def mock_yaml_loader(filename):

                if 'policy.yaml' in filename:

                    return {def test_jitter_config_disabled(tmp_path):

                        'loop': {'interval_seconds': 60, 'jitter_pct': -10.0},  # Negative    """Test jitter can be disabled with 0%."""

                        'risk': {'max_total_at_risk_pct': 15.0},    config_dir = tmp_path / "config"
                ...
                return {'triggers': {}} if 'signals' in filename else {'tiers': {}}        "app": {"mode": "DRY_RUN"},

                    "exchange": {"api_key": "test", "api_secret": "test", "read_only": True},

            mock_load.side_effect = mock_yaml_loader        "logging": {"level": "INFO"}

                }))

            with patch('tools.config_validator.validate_all_configs', return_value=[]):    

                loop = TradingLoop(config_dir="config")    (config_dir / "policy.yaml").write_text(yaml.dump({
...
class TestJitterApplication:        "latency": {"api_thresholds_ms": {}, "stage_budgets": {}, "total_seconds": 45.0}

    """Test jitter is applied correctly during loop execution."""    }))

        

    @patch('runner.main_loop.time.sleep')    (config_dir / "signals.yaml").write_text(yaml.dump({"triggers": {}}))

    @patch('runner.main_loop.time.monotonic')    (config_dir / "universe.yaml").write_text(yaml.dump({"tiers": {}}))

    def test_jitter_applied_in_valid_range(self, mock_monotonic, mock_sleep):    

        """Test jitter adds 0 to jitter_pct% to sleep duration."""    loop = TradingLoop(config_dir=str(config_dir))

        # Simulate cycle taking 5s    assert loop.loop_jitter_pct == 0.0

        mock_monotonic.side_effect = [0.0, 5.0]

        

        loop = TradingLoop(config_dir="config")def test_jitter_clamped_max(tmp_path):
        ...
        loop.state_store = Mock()        "app": {"mode": "DRY_RUN"},

        loop.state_store.load = Mock(return_value={})        "exchange": {"api_key": "test", "api_secret": "test", "read_only": True},

        loop.state_store.save = Mock()        "logging": {"level": "INFO"}

            }))

        def stop_after_one():    

            loop._running = False    (config_dir / "policy.yaml").write_text(yaml.dump({

        loop.run_cycle.side_effect = stop_after_one        "loop": {"interval_seconds": 60, "jitter_pct": 50.0},  # Try to set 50%

                "risk": {"max_total_at_risk_pct": 15.0},

        sleep_durations = []        "execution": {"maker_fee_bps": 40, "taker_fee_bps": 60},

        def capture_sleep(duration):        "latency": {"api_thresholds_ms": {}, "stage_budgets": {}, "total_seconds": 45.0}
        ...
        loop.run(interval_seconds=60)    (config_dir / "universe.yaml").write_text(yaml.dump({"tiers": {}}))

            

        assert len(sleep_durations) == 1    loop = TradingLoop(config_dir=str(config_dir))

        sleep_duration = sleep_durations[0]    assert loop.loop_jitter_pct == 20.0  # Clamped to 20%

        

        # With 10% jitter and 60s interval:

        # Base sleep = 60 - 5 = 55sdef test_jitter_clamped_negative(tmp_path):

        # Jitter adds 0 to 6s (10% of 60)    """Test jitter clamped to minimum 0%."""

        # Total should be [55, 61]    config_dir = tmp_path / "config"
