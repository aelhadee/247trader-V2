...
def test_shadow_execution_stale_quote(execution_engine, mock_exchange, shadow_log_file):
    ...
    # Mock stale quote
    stale_quote = create_test_quote(timestamp=datetime.now(timezone.utc) - timedelta(seconds=60))
...
def test_shadow_execution_wide_spread(execution_engine, mock_exchange, shadow_log_file):
    ...
    # Mock wide spread quote (120bps)
    wide_quote = create_test_quote(bid=50000.0, ask=50600.0)
